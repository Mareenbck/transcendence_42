...
import type { gameInit, gameState, gameStatus, gameList, UserGame, players } from './interface_game'
        ...
        const handleGameRooms = (games:gameList[]) => {
            ...
            if (games){
                ...
                if (index != -1){
                    ...
                    setPlayers({playerL: game.playerL, playerR: game.playerR});
                }
            }

        }
        addListener("gameRooms", handleGameRooms);
        sendMessage('listRooms', { user: user.userId, status: gamestatus.status } as any);    

 console.log("playerL et playerR + winner", gamestatus.playerR, gamestatus.playerL, gamestatus.winner);
    }, []);

// event: Game's page entry
    useEffect(() => {
        sendMessage("enterGame", {user: user.userId} as any);
        return () => {
...
// action when clicking on "PlayGame"
    ...
    const handleClick = (roomN: number) => {
        sendMessage("playGame", {user: user.userId, roomN: roomN} as any);
        setCurRoom(roomN);
        if(roomN == -1){
            gamestatus.status == 'game';
        }
        ...
        <div className="containerGame">
            <div className="gameSideL">
                ...
                <div className='posBtnn' >
                    ...
                    <Link to="/menu">
                        <button className="btnn" onClick={() => sendMessage("exitGame", { user: user.userId, status: gamestatus.status } as any)} style={{ alignSelf: "flex-end" }}>
                            <ExitToAppIcon /> Exit
                        </button>
                    </Link>
                        ...
                        {/* <div className='wrapWaiting'>*/}
                            ...
                            { (gamestatus.status == 'watch' || gamestatus.status == 'game') ? 
                                (// getCurrentGame(curroom);
                                    <HeaderGame playerL = {players.playerL} playerR = {players.playerR}/>
                                )
                                :  
                    ...
                    <>
                        ...
                        <section className='sctWin'>
                            ...
                            <div className="container-match" style={{backgroundColor: "white",
                                                                    border: "solid" ,
                                                                    borderBlockColor:"black" } } >
                                {/* <PlayerOne  style={{backgroundColor: "white"}} player={gamestatus.playerL} winner={""} sizeAvatar={"l"} />
                                <p className='vs'> VS</p>
                            
                                <PlayerTwo  player={gamestatus.playerR} winner={""}  sizeAvatar={"l"} /> */}
                                <HeaderGame playerL={gamestatus.winner = gamestatus.playerL ? gamestatus.winner : gamestatus.playerL} playerR={gamestatus.winner = gamestatus.playerR ? gamestatus.winner : gamestatus.playerR} />
